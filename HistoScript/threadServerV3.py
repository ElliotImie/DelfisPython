...
class ThreadVelo(Thread):
	...
	def run(self):
		...
		self.cursor.execute("""
			CREATE TABLE IF NOT EXISTS usr_29(
		...
		while True :
			...
			self.cursor.execute("INSERT INTO usr_29 (id,latitude,longitude) VALUES (\" " + message["id_user"] + " \" , \" "+ message["latitude"] + " \" , \" "+ message["longitude"] +" \" );")
			self.connBDD.commit()
			print("insert OK")
			self.connBDD.close
